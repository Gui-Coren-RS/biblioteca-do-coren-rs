...
import { Book, UserPlus, Search, User, FileText, Send, XCircle, Clock } from 'lucide-react';
...
export function Library() {
  ...
  useEffect(() => {
    ...
    // Fetch all books that are available or in process, and NOT owned by the current user
    const q = query(
      collection(db, 'books'),
      where('status', 'in', ['disponível', 'em processo de empréstimo'])
    );
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500">Livros disponíveis para empréstimo</p>
        </div>
      ...
      <ConfirmModal
        ...
        title={confirmAction?.type === 'borrow' ? 'Solicitar Empréstimo' : 'Cancelar Solicitação'}
        message={confirmAction?.type === 'borrow' ? 'Deseja solicitar o empréstimo deste livro? O dono será notificado.' : 'Deseja cancelar sua solicitação de empréstimo?'}
        confirmText="Sim"
        cancelText="Não"
